...
grep -Fq 'local_spec = false' "$init"
grep -Fq "nvim --headless '+Lazy! restore' '+Lazy! clean' +qa" "$repo_root/scripts/phases/60-neovim.sh"
grep -Fq 'nvim --headless "+Lazy! update" +qa' "$repo_root/scripts/update-neovim-lock.sh"
