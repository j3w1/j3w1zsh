...
This active-managed repository uses profile `personal-tooling-adjacent`; protected CE label
prefixes are `ce-systems`, `cross-repo`, `historical-evidence`, `type:`, `area:`, and `concern:`.
Project membership is currently `MANUAL_OWNER`; the protected post-cutover target is
...
rollout authority. Do not create competing label, dashboard, or Project automation. Its output never
grants source, task, review, merge, release, deployment, or production authority. Preserve the
provider-view hold for `j3w1zsh#22`; never toggle otherwise-correct metadata to force view membership.
